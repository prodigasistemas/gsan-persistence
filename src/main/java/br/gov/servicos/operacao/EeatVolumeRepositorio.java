...
@Stateless
public class EeatVolumeRepositorio extends GenericRepository<Integer, EEATVolume>{

	public List<EEATVolume> obterLista() {
	...
	public boolean verificaMesReferencia(Integer codigo, String mesReferencia) throws Exception {
		TypedQuery<EEATVolume> query = entity.createQuery("select c1 from EEATVolume c1 where eeat_id = " + codigo + " AND eatv_referencia = '" + mesReferencia
				+ "'", EEATVolume.class);
	...
	public List<EEATVolume> obterListaLazy(int startingAt, int maxPerPage, Map<String, Object> filters) throws Exception {
	...
	public int obterQtdRegistros(Map<String, Object> filters) throws Exception {
		 ...
		 * Join<EEATVolume, UnidadeNegocioProxy> uneg =
		 * c.join("unidadeNegocioProxy"); Join<EEATVolume, MunicipioProxy> muni
		 * = c.join("municipioProxy"); Join<EEATVolume, LocalidadeProxy> loca =
