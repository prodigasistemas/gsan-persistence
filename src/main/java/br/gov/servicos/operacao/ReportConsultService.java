...
import br.gov.model.operacao.TotalMensal;
import br.gov.model.operacao.TotalProdutoQuimicoMensal;
...
public class ReportConsultService {
	...
	public static Collection<TotalProdutoQuimicoMensal> getTotais(String restriction, Date dataIni, Date dataFim) {
		...
		builder.append("SELECT C.prod_nmproduto as produto, ").append(" substr(cast(B.cons_data as text), 1, 7) as referencia,")
				...
				.append(" E.muni_id, E.muni_nmmunicipio, C.loca_id, C.loca_nmlocalidade ").append(" FROM cadastro.gerencia_regional A ")
				...
				.append("            A.cons_data, B.prod_id, B.conp_quantidade").append("            FROM operacao.consumoeta A")
				.append("            INNER JOIN operacao.consumoeta_produto B ON A.cons_id = B.cons_id")
				.append("            INNER JOIN operacao.eta C ON A.eta_id = C.eta_id").append("            UNION ALL ")
				...
				.append("                   A.cons_data, B.prod_id, B.conp_quantidade").append("            FROM operacao.consumoeat A")
				.append("            INNER JOIN operacao.consumoeat_produto B ON A.cons_id = B.cons_id")
				.append("            INNER JOIN operacao.eeat C ON A.eat_id = C.eeat_id").append("            )")
				.append("  B ON A.greg_id = B.greg_id AND A.uneg_id = B.uneg_id AND A.muni_id = B.muni_id AND A.loca_id = B.loca_id")
				.append(" INNER JOIN operacao.produto C ON B.prod_id = C.prod_id ").append(" INNER JOIN operacao.unidademedida D ON C.umed_id = D.umed_id")
				.append(" WHERE B.cons_data BETWEEN ").append("'" + format.format(dataIni) + "'").append(" AND ").append("'" + format.format(dataFim) + "'")
				.append("  AND B.conp_quantidade <> 0").append("  AND ").append(restriction)
		...
		try {
			...
			DataSource dataSource = (DataSource) ctx.lookup("java:/jboss/datasources/GsanDS");
