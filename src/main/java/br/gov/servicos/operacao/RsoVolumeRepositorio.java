...
@Stateless
public class RsoVolumeRepositorio extends GenericRepository<Integer, RSOVolume>{
	
	public List<RSOVolume> obterLista() {
	...
	public boolean verificaMesReferencia(Integer codigo, String mesReferencia) throws Exception {
		TypedQuery<RSOVolume> query = entity.createQuery("select c1 from RSOVolume c1 where rso_id = " + codigo + " AND rsov_referencia = '" + mesReferencia
				+ "'", RSOVolume.class);
	...
	public List<RSOVolume> obterListaLazy(int startingAt, int maxPerPage, Map<String, Object> filters) throws Exception {
	...
	public int obterQtdRegistros(Map<String, Object> filters) throws Exception {
		 ...
		 * Join<RSOVolume, UnidadeNegocioProxy> uneg =
		 * c.join("unidadeNegocioProxy"); Join<RSOVolume, MunicipioProxy> muni =
		 * c.join("municipioProxy"); Join<RSOVolume, LocalidadeProxy> loca =
