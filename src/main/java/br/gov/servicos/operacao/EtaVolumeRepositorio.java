...
@Stateless
public class EtaVolumeRepositorio extends GenericRepository<Integer, ETAVolume>{
	public List<ETAVolume> obterLista() {
	...
	public boolean verificaMesReferencia(Integer codigo, String mesReferencia) throws Exception {
		TypedQuery<ETAVolume> query = entity.createQuery("select c1 from ETAVolume c1 where eta_id = " + codigo + " AND etav_referencia = '" + mesReferencia
				+ "'", ETAVolume.class);
	...
	public List<ETAVolume> obterListaLazy(int startingAt, int maxPerPage, Map<String, Object> filters) throws Exception {
	...
	public int obterQtdRegistros(Map<String, Object> filters) throws Exception {
		 ...
		 * Join<ETAVolume, UnidadeNegocioProxy> uneg =
		 * c.join("unidadeNegocioProxy"); Join<ETAVolume, MunicipioProxy> muni =
		 * c.join("municipioProxy"); Join<ETAVolume, LocalidadeProxy> loca =
