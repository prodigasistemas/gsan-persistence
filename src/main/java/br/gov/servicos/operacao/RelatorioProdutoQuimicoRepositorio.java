...
import br.gov.model.operacao.ElementoMensal;
...
public class RelatorioProdutoQuimicoRepositorio {

	@EJB
	ProxyOperacionalRepositorio fachadaProxy;

	public List<RelatorioGerencial> getConsumoProdutoSintetico(Integer codigoRegional, Integer codigoUnidadeNegocio, Integer codigoMunicipio,
			Integer codigoLocalidade, Integer tipoUnidadeOperacional, Integer codigoUnidadeOperacional, Date dataInicial, Date dataFinal,
			Integer tipoAgrupamento, Integer tipoRelatorio) throws Exception {
		...
		if (tipoAgrupamento >= 3) {// GERENCIA REGIONAL + UNIDADE NEGOCIO +
									// MUNICIPIO
			query = query + " A.muni_id, A.muni_nmmunicipio,";
		}
		if (tipoAgrupamento >= 4) {// GERENCIA REGIONAL + UNIDADE NEGOCIO +
									// MUNICIPIO + LOCALIDADE
			query = query + " A.loca_id, A.loca_nmlocalidade,";
		}
		if (tipoAgrupamento >= 5) {// GERENCIA REGIONAL + UNIDADE NEGOCIO +
									// MUNICIPIO + LOCALIDADE + SISTEMA
									// ABASTECIMENTO
		...
		if (codigoUnidadeOperacional != -1) {
			query = query + "  AND B.unop_id = " + codigoUnidadeOperacional + "  AND B.unop_tipo = " + tipoUnidadeOperacional;
		}
	...
	public List<RelatorioGerencial> getConsumoProdutoMensal(Integer codigoRegional, Integer codigoUnidadeNegocio, Integer codigoMunicipio,
			Integer codigoLocalidade, Integer tipoUnidadeOperacional, Integer codigoUnidadeOperacional, Date dataInicial, Date dataFinal,
			Integer tipoAgrupamento, Integer tipoRelatorio) throws Exception {
		...
		if (codigoUnidadeOperacional != -1) {
			query.append("  AND B.unop_id = " + codigoUnidadeOperacional + "  AND B.unop_tipo = " + tipoUnidadeOperacional);
		}
	...
	public List<RelatorioGerencial> getConsumoProdutoAnalitico(Integer codigoRegional, Integer codigoUnidadeNegocio, Integer codigoMunicipio,
			Integer codigoLocalidade, Integer tipoUnidadeOperacional, Integer codigoUnidadeOperacional, Date dataInicial, Date dataFinal,
			Integer tipoAgrupamento, Integer tipoRelatorio) throws Exception {
		...
		query += " ORDER BY ";
		query += Utilitarios.ordenarCamposConsulta(tipoAgrupamento);
	...
	/*
	 * public List<Consumo> getUnidadeOperacionalUsuario(UsuarioProxy usuario)
	 * throws Exception {
	 * 
	 * String localidade = ""; String query =
	 * "SELECT A.* FROM (SELECT DISTINCT A.greg_id, A.greg_nmregional, B.uneg_id, B.uneg_nmunidadenegocio,"
	 * +
	 ...
	 * ;
	 * 
	 * //Se Perfil de Gerente, acesso a gerência Regional if
	 * (usuario.getPerfil() == PerfilBeanEnum.GERENTE){ query = query +
	 * " WHERE A.greg_id = " + usuario.getRegionalProxy().getCodigo(); } else if
	 * (usuario.getPerfil() == PerfilBeanEnum.SUPERVISOR || usuario.getPerfil()
	 * == PerfilBeanEnum.COORDENADOR){ for (LocalidadeProxy colunas :
	 * usuario.getLocalidadeProxy()) { localidade = localidade +
	 * colunas.getCodigo() + ","; } localidade = localidade.substring(0,
	 * localidade.length() - 1); query = query + " WHERE A.loca_id IN (" +
	 * localidade + ")"; }
	 * 
	 * List<List> valores = fachadaProxy.selectRegistros(query); List<Consumo>
	 * lista = new ArrayList<Consumo>();
	 * 
	 * if (valores == null) { return lista; }
	 * 
	 * Integer intI = 1; for (List colunas : valores) { Consumo consumo = new
	 * Consumo(); consumo.setCodigo(intI++); consumo.setRegionalProxy(new
	 * RegionalProxy
	 * (Integer.parseInt(colunas.get(0).toString()),colunas.get(1).toString()));
	 * consumo.setUnidadeNegocioProxy(new
	 * UnidadeNegocioProxy(Integer.parseInt(colunas
	 * .get(2).toString()),colunas.get(3).toString()));
	 * consumo.setMunicipioProxy(new
	 * MunicipioProxy(Integer.parseInt(colunas.get(
	 * 4).toString()),colunas.get(5).toString()));
	 * consumo.setLocalidadeProxy(new
	 * LocalidadeProxy(Integer.parseInt(colunas.get
	 * (6).toString()),colunas.get(7).toString()));
	 * consumo.setUnidadeOperacionalProxy(new
	 * UnidadeOperacionalProxy(Integer.parseInt
	 * (colunas.get(8).toString()),colunas.get(9).toString()));
	 * lista.add(consumo); }
	 * 
	 * return lista; }
	 */
