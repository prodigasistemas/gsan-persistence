...
@Stateless
public class EteVolumeRepositorio extends GenericRepository<Integer, ETEVolume>{
	public List<ETEVolume> obterLista() {
	...
	public boolean verificaMesReferencia(Integer codigo, String mesReferencia) throws Exception {
		TypedQuery<ETEVolume> query = entity.createQuery("select c1 from ETEVolume c1 where ete_id = " + codigo + " AND etev_referencia = '" + mesReferencia
				+ "'", ETEVolume.class);
	...
	public List<ETEVolume> obterListaLazy(int startingAt, int maxPerPage, Map<String, Object> filters) throws Exception {
		...
		// Join<ConsumoETE, UnidadeNegocioProxy> uneg =
		...
		if (filters != null && !filters.isEmpty()) {
			...
			for (Map.Entry<String, Object> entry : filters.entrySet()) {
				...
				try {
					 ...
					 * loca.get("nome");
					 */
					if (key.equals("ete.descricao"))
						path = ete.get("descricao");
					else
						path = c.get(key);
	...
	public int obterQtdRegistros(Map<String, Object> filters) throws Exception {
		 ...
		 * Join<ETEVolume, UnidadeNegocioProxy> uneg =
		 * c.join("unidadeNegocioProxy"); Join<ETEVolume, MunicipioProxy> muni =
		 * c.join("municipioProxy"); Join<ETEVolume, LocalidadeProxy> loca =
