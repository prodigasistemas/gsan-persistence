...
public class IndicadorRepositorio {
	...
	public List<Indicador> getIndicadorMensal(Integer codigoRegional, Integer codigoUnidadeNegocio, Integer codigoMunicipio, Integer codigoLocalidade,
		...
		query.append(" SELECT ");
		query.append(campos).append(" , SUM(B.indm_vlr1) AS indm_vlr1, SUM(B.indm_vlr2) AS indm_vlr2, SUM(B.indm_vlrtotal) AS indm_vlrtotal ")
				.append(" FROM operacao.indicador A ").append(" INNER JOIN operacao.indicador_mensal B ON A.indc_id = B.indc_id ")
				...
				.append("   unop.unop_tipo = B.unop_tipo AND unop.unop_id = B.unop_id ").append(" WHERE A.indc_id IN (" + listaIndicador + ")").append(filtro)
				.append(grupo).append(ordem);
		...
		while (rs.next()) {
			if (rs.getString("indm_data") != null) {
				...
				if (existeIndicadorNaLista(lista, indicador)) { // INDICADOR NAO
																// EXISTE NA
																// LISTA
	...
	private Indicador recuperaIndicadorNaLista(List<Indicador> lista, Indicador indicador) {
		...
		for (Integer i = 0; i < lista.size(); i++) {
			...
			if (mesmoCodigo(item, indicador) && mesmaRegional(item, indicador) && mesmaUnidadeNegocio(item, indicador) && mesmoMunicipio(item, indicador)
					&& mesmaLocalidade(item, indicador) && mesmaUnidadeOperacional(item, indicador)) {
				retorno = item;
				break;
			}
		}
